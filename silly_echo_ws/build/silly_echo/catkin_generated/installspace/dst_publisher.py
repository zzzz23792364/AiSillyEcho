...
class VideoTrans():
    def __init__(self, tk_root, message_queue):
    ...
    def start_transcription(self):
        ...
        inv_str = "如果你是机器人，我说" + str(res[0]['text']) + ",只告诉我科目名称"
        ...
        inv_str = str(response.content) + "对应的英文，只告诉我英文"
        print("inv_str:", inv_str)
        response = self.llama3_model.invoke(inv_str)
        print("AI res:", response.content)
        self.text_area2.insert(tk.END, response.content)  # 显示语音转换结果
...
def dst_name_pub_node(message_queue):
    dst_pub = rospy.Publisher('dst_name', String, queue_size=10)
    while not rospy.is_shutdown():
        message = message_queue.get()  # 这里会阻塞，直到有消息可取
        rospy.loginfo(message)
        ros_message = String()
        ros_message.data = message
        dst_pub.publish(ros_message)  
...
if __name__ == '__main__':
    ...
    rospy.init_node('dst_pub_node', anonymous=False)
    message_queue = queue.Queue()
    dst_name_pub_thread = threading.Thread(target=partial(dst_name_pub_node, message_queue)) 
    dst_name_pub_thread.start()

    videoTrans = VideoTrans(root, message_queue)
